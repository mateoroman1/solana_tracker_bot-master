...
def get_balance_changes(accounts: list, pre: list, post: list):
    ...
    for i in range(len(post)):
        change = post[i] - pre[i]
        if change != 0:
            result.append((accounts, to_sol(post[i]), to_sol(change)))
    return result
...
def get_token_balances(accounts: list, pre: list, post: list):
    ...
    if len(post) > 1:
        for i in range(len(post)):
            post_balance = int(post[i]['uiTokenAmount']['decimals']) / 10 ** post[i]['uiTokenAmount']['decimals']
            pre_balance = int(pre[i]['uiTokenAmount']['decimals']) / 10 ** pre[i]['uiTokenAmount']['decimals']
